...
def char_cnn_model(x):
    ...
    with tf.variable_scope('CNN_Layer1'):
        ...
        pool1 = tf.layers.max_pooling2d(
            ...
            padding='SAME')
        pool_1_dropout = tf.nn.dropout(pool1, keep_prob)
    ...
    with tf.variable_scope('CNN_Layer2'):
        ...
        pool2 = tf.layers.max_pooling2d(
            ...
            strides=POOLING_STRIDE,
            padding='SAME')
        pool_2_dropout = tf.nn.dropout(pool2, keep_prob)

        pool2 = tf.squeeze(tf.reduce_max(pool_2_dropout, 1), squeeze_dims=[1])
...
def word_cnn_model(x, n_words):
  ...
  with tf.variable_scope('CNN_Layer4'):
      ...
      logits = tf.squeeze(tf.reduce_max(pool2_dropout, 1), axis=[1])

  # Apply regular WX + B and classification.
  logits = tf.layers.dense(logits, MAX_LABEL, activation=None)
